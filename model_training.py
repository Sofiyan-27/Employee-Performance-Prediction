...
Usage:
  python model_training.py --data data/garments_worker_productivity.csv --target productivity
...
def prepare_dataframe(df):
    ...
    # Strip whitespace & lower case for object/string columns (helps unify categories)
...
def train_and_select(X_train, X_test, y_train, y_test, preprocessor):
    models = {
        ...
        'XGBoost': xgb.XGBRegressor(n_estimators=100, random_state=42, objective='reg:squarederror', verbosity=0)
    }
...
def main(args):
    ...
    # 2) Drop columns with extremely many missing values (>60%)
    thresh = 0.6 * len(df)
